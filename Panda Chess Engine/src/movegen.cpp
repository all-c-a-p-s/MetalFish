...
const int max_legal_moves = 100;

bool ischeck(int colour_to_test, int board[], int board_10x12[]);
...
class piece{
    ...
    public:
            ...
            while(found_count < 10){//there can never be more than 10 of a certain piece on the board at a time
                for(int i = 0; i <64; i++){
                    if(board[i] == board_code){
                        squares_found[found_count] = get_10x12(i);//where i is the square. this is 10x12'd because it passes directly into movegen()
                        found_count ++;
                    }
                }
                break;
        ...
        void movegen(int square, int board[], int board_10x12[]){//function to update moves array with all pseudolegal moves in a position
            ...
            if(slider == true){
                ...
                square = temp;//reset square to the value passed into the function
                    ...
                    while((board_10x12[square + moveset[i]] == -6) or (board_10x12[square + moveset[i]] == 30) or (isenemy(board_code,board_10x12[square + moveset[i]]) == true)){//pieces can also move onto en passant squares
                        ...
                        if(isenemy(board_code,board_10x12[square + moveset[i]]) == true){
                            break;
                        }
                ...
                for(int i = 0; i < 4; i++){
                    if(board_10x12[square + moveset[i]] == -6 or board_10x12[square + moveset[i]] == 30 or isenemy(board_code,board_10x12[square + moveset[i]]) == true){
                        pseudolegal_move move;
                ...
                if(board_code == 5 and moveset[0] == 10){//otherwise this will get added for both king1 and king2
                    if(white_kingside_castling == true and ischeck(5, board, board_10x12) == false){
                        if(board_10x12[get_10x12(5)] == -6 and board_10x12[get_10x12(6)] == -6){//if nothing between king and rook
                            pseudolegal_move move;
                            move.square_from = get_8x8(square);
                    ...
                    if(white_queenside_castling == true and ischeck(5, board, board_10x12) == false){
                        if(board_10x12[get_10x12(3)] == -6 and board_10x12[get_10x12(2)] == -6 and board_10x12[get_10x12(1)] == -6){//if nothing between king and rook
                            pseudolegal_move move;
                            move.square_from = get_8x8(square);
                ...
                else if(board_code == 29 and moveset[0] == 10){//as above
                    if(black_kingside_castling == true and ischeck(29, board, board_10x12) == false){
                        if(board_10x12[get_10x12(61)] == -6 and board_10x12[get_10x12(62)] == -6){//if nothing between king and rook
                            pseudolegal_move move;
                            move.square_from = get_8x8(square);
                    ...
                    if(black_queenside_castling == true and ischeck(29, board, board_10x12) == false){
                        if(board_10x12[get_10x12(59)] == -6 and board_10x12[get_10x12(58)] == -6 and board_10x12[get_10x12(57)] == -6){//if nothing between king and rook
                            pseudolegal_move move;
                            move.square_from = get_8x8(square);
                ...
                if((get_10x12_rank(square)== 4 and get_colour(board_code) == 0) or (get_10x12_rank(square)== 9 and get_colour(board_code) == 4)){
                //if pawn hasn't moved yet
                    if(board_10x12[square + moveset[0]] == -6 and board_10x12[square + moveset[1]] == -6){
                ...
                if(board_10x12[square + moveset[0]] == -6){//if pawn is not blocked
                    pseudolegal_move move;
                    move.square_from = get_8x8(square);
                ...
                if(isenemy(board_code,board_10x12[square + moveset[2]]) == true or board_10x12[square + moveset[2]] == 30){
                    pseudolegal_move move;
                    move.square_from = get_8x8(square);
                ...
                if(isenemy(board_code,board_10x12[square + moveset[3]]) == true or board_10x12[square + moveset[3]] == 30){
                    pseudolegal_move move;
                    move.square_from = get_8x8(square);
        ...
        void get_moves(int board[], int board_10x12[]){//function which calls find_piece() and movegen()
            find_piece(board);
            ...
            for(int i = 0;i < 10;i++){
                ...
                movegen(squares_found[i], board, board_10x12);
...
bool ischeck(int colour_to_test, int board[], int board_10x12[]){//function to determine whether current position if check
    ...
    for(int i = 0;i < 64;i++){
        if(board[i] == colour_to_test){
            king_square = get_10x12(i);//get_10x12() because this will be passed into movegen
        }
    }
    ...
    if(colour_to_test == 5){
        if(board_10x12[king_square + 11] == 24 or board_10x12[king_square + 9] == 24){
        ...
        if(board_10x12[king_square - 11] == 0 or board_10x12[king_square - 9] == 0){
            moves_count = moves_count_reset;
            moves = moves_reset;
    ...
    for(int i = 0;i < 8;i++){
        if(get_type(board_10x12[king_square + knight_moveset[i]]) == 1 and isenemy(colour_to_test, board_10x12[king_square + knight_moveset[i]])){
            moves_count = moves_count_reset;
            moves = moves_reset;
    ...
    for(int i = 0;i < 8;i++){
        int product = colour_to_test * board_10x12[king_square + king_moveset[i]];//test if kings are next to each other
    ...
    queen_check_1.get_moves(board, board_10x12);
    ...
    queen_check_2.get_moves(board, board_10x12);

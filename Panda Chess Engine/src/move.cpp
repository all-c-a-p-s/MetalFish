...
class Move{//Move object contains all the information required to make/undo a move on the board
    ...
    void make_move(int board[], int board_12x12[]){//function to make a move on the board      
        ...
        if(type == normal){
            ...
            board_12x12[get_12x12(square_to)] = promote_to;
...
void get_legal(int board[], int board_12x12[]){//generates array of all legal moves in a position
    
    int checking_board[64];
    int checking_12x12[144];

    for(int i = 0;i < 64;i++){
        checking_board[i] = board[i];
    }

    for(int i = 0;i < 144;i++){
        checking_12x12[i] = board_12x12[i];
    }
    ...
    if(white_to_move(tempi) == true){
        ...
        get_w_pseudolegal(checking_board, checking_12x12);//first get white's pseudolegal moves
        ...
        for(int i = 0; i < 100; i++){
            ...
            int sq_reset = checking_board[sq_to];
            int piece_moved = checking_board[sq_from];

            checking_board[sq_to] = checking_board[sq_from];
            checking_board[sq_from] = -6;//update board

            checking_12x12[get_12x12(sq_to)] = checking_12x12[get_12x12(sq_from)];
            checking_12x12[get_12x12(sq_from)] = -6;//update board_12x12

            //we don't need to worry about updating castling rights etc as this won't change whether the move if legal or not
            
            if(ischeck(5,checking_board, checking_12x12) == false){//if move doesn't leave us in check
                int promote_counter;
                promote_counter = 0;
            ...
            checking_board[sq_from] = checking_board[sq_to];
            checking_board[sq_to] = sq_reset;//reset board

            checking_12x12[get_12x12(sq_from)] = checking_12x12[get_12x12(sq_to)];
            checking_12x12[get_12x12(sq_to)] = sq_reset;//reset checking_12x12
        }

    }

    else{//black to move
        get_b_pseudolegal(checking_board, checking_12x12);//first get white's pseudolegal moves
        ...
        for(int i = 0; i < 100;i++){
            ...
            checking_12x12[get_12x12(sq_to)] = checking_12x12[get_12x12(sq_from)];
            checking_12x12[get_12x12(sq_from)] = -6;//update checking_12x12

            if(ischeck(29,checking_board, checking_12x12) == false){//if move doesn't leave us in check
            ...
            checking_12x12[get_12x12(sq_from)] = checking_12x12[get_12x12(sq_to)];
            checking_12x12[get_12x12(sq_to)] = sq_reset;
        }

...
enum move_type{normal, promotion};

void get_w_pseudolegal(int board[], int board_12x12[]){
...
void get_b_pseudolegal(int board[], int board_12x12[]){
...
class Move{
    ...
    void make_move(int board[], int board_12x12[]){
        for(int i = 0; i < 64; i++){
            if(board[i] == 30){
                board[i] = -6;
        ...
        if(ep_square != -1){
            board[ep_square] = -6;
            board_12x12[get_12x12(ep_square)] -6;
            en_passant_square = -1;
        }
        */
    ...
    void undo_move(int board[], int board_12x12[]){
        if(type == normal){
            if(square_from == 4 and board[square_from] == 5 and square_to == 6){//if white castles kingside
                board[4] = 5;
                board[7] = 3;
...
array<Move, 218> legal_moves;


bool ischeck(int colour_to_test, int board[], int board_12x12[]){

    array <pseudolegal_move, 218> check_moves = moves;
    ...
    if(colour_to_test == 5){
        ...
        for(int i = 0;i < 218;i++){
            ...
            if(board[move.square_to] == 29){//if any of the moves land on white's king square
                return true;//white is in check
            }            
        }
    }

    return false;//passed through loop and no checks found
    

}

void get_legal(int board[], int board_12x12[]){
